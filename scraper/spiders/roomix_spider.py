...
class RoomixSpider(scrapy.Spider):
    ...
    custom_settings = {
        'DOWNLOAD_DELAY': 2,  # Respetar el servidor
        'CONCURRENT_REQUESTS': 4,  # Reducir carga
        'SELENIUM_ENABLED': False,
        'USER_AGENT': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
    }
    
    def start_requests(self):
        # URLs con filtros: casas y PH, Rosario, $300k-800k, con patio
        urls = [
    ...
    def parse_property(self, response):
        ...
        try:
            ...
            # Estrategia 1: Buscar patrón Calle + Altura en el Título (Fuente más confiable en Roomix)
            if titulo:
                # Regex mejorada: Busca palabras Capitalizadas o MAYÚSCULAS seguidas de un número
                # Ej: San Juan 4770, SAN JUAN 4770, Mendoza 1200
                match = re.search(r'\b([A-ZÁÉÍÓÚÑ][a-záéíóúñA-ZÁÉÍÓÚÑ]+(?:\s+(?:de|del|la|las|el|los|y|DE|DEL|LA|LAS|EL|LOS|Y)\s+|[A-ZÁÉÍÓÚÑ][a-záéíóúñA-ZÁÉÍÓÚÑ]+\s*)+)\s+([0-9]{2,5})\b', titulo)
                if match:
                    direccion = f"{match.group(1)} {match.group(2)}"
            
            # Estrategia 2: Si falla titulo, buscar en selectores de dirección
            if not direccion:
                dir_text = response.css('address::text, .address::text, .location::text').get()
                if dir_text and any(char.isdigit() for char in dir_text): # Solo si tiene números
                     direccion = dir_text.strip()

            # Estrategia 3: Buscar en texto completo si aún no hay dirección
            if not direccion:
                 match = re.search(r'\b([A-ZÁÉÍÓÚÑ][a-záéíóúñ]+(?:\s+[A-ZÁÉÍÓÚÑ][a-záéíóúñ]+)*)\s+([0-9]{2,5})\b', response.text)
                 if match:
                     direccion = f"{match.group(1)} {match.group(2)}"
