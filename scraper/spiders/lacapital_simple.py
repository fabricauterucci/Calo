...
class LaCapitalSimpleSpider(scrapy.Spider):
    """
    Spider optimizado de La Capital - extrae desde el listado
    """
    name = 'lacapital_simple'
    allowed_domains = ['lacapital.com.ar']
    ...
    def parse_listing(self, response):
        ...
        propiedades = response.css('div.propiedad, div[class*="property"], article, div.item-propiedad')
        ...
        for prop in propiedades:
            ...
            ubicacion = (
                ...
                prop.css('.barrio::text, .zona::text').get()
            )
            ...
            # PRECIO
            precio_text = prop.css('[class*="precio"]::text, [class*="price"]::text, .valor::text').get()
            if precio_text:
        ...
        # Paginación
        next_page = response.css('a.siguiente::attr(href), a[rel="next"]::attr(href), a.pagination-next::attr(href)').get()
        if next_page:
