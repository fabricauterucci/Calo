...
class ArgenpropSpider(scrapy.Spider):
    ...
    def start_requests(self):
        # URLs ampliadas para traer más resultados
        # Los filtros estrictos del pipeline se encargarán de la calidad
        urls = [
            # PHs en Rosario - diferentes rangos de precio
            'https://www.argenprop.com/ph/alquiler/rosario-o-rosario-santa-fe/pesos-300000-600000?solo-ver-pesos',
            'https://www.argenprop.com/ph/alquiler/rosario-o-rosario-santa-fe/pesos-600000-900000?solo-ver-pesos',
            
            # Casas en Rosario - diferentes rangos y con/sin patio
            'https://www.argenprop.com/casas/alquiler/rosario-o-rosario-santa-fe/pesos-300000-600000?solo-ver-pesos',
            'https://www.argenprop.com/casas/alquiler/rosario-o-rosario-santa-fe/pesos-600000-900000?solo-ver-pesos',
            'https://www.argenprop.com/casas/alquiler/rosario-o-rosario-santa-fe/pesos-300000-600000?con-ambiente-patio&solo-ver-pesos',
            'https://www.argenprop.com/casas/alquiler/rosario-o-rosario-santa-fe/pesos-600000-900000?con-ambiente-patio&solo-ver-pesos',
        ]
    ...
    def parse_propiedad(self, response):
        ...
        if '/ph/' in url_lower or '/ph-' in url_lower:
            ...
            # Intentar desde el título o breadcrumbs
            tipo_text = response.css('span.breadcrumb-item::text').getall()
            tipo_str = ' '.join(tipo_text).lower()
            if 'ph' in tipo_str:
                item['tipo'] = 'PH'
            elif 'casa' in tipo_str:
                item['tipo'] = 'Casa'
            elif 'departamento' in tipo_str:
                item['tipo'] = 'Departamento'
        ...
        # Dirección - extractor mejorado
        # Intenta obtener la dirección de varios selectores posibles
        direccion = (
            response.css('h3.card__address::text').get() or
            response.css('p.card__address::text').get() or
            response.css('h2.titlebar__address::text').get() or
            response.css('span.titlebar__address::text').get()
        )
        
        # Si no encuentra dirección explícita, intenta buscarla en el título
        if not direccion:
            titulo = response.css('h1::text').get() or response.css('h2::text').get() or ''
            # Regex para buscar Calle + Numero (ej: Mitre 1200, San Juan 400, SAN JUAN 400)
            import re
            match = re.search(r'\b([A-ZÁÉÍÓÚÑ][a-záéíóúñA-ZÁÉÍÓÚÑ]+(?:\s+[A-ZÁÉÍÓÚÑ][a-záéíóúñA-ZÁÉÍÓÚÑ]+)*)\s+(\d{1,5})', titulo)
            if match:
                direccion = f"{match.group(1)} {match.group(2)}"
        
        if direccion:
            item['direccion'] = direccion.strip()
            # Si la dirección parece válida, intentar usarla como título si no hay uno mejor
        else:
            item['direccion'] = ''
        ...
        # Título descriptivo: construir al final usando los datos ya extraídos
        partes = []
        # Usar el h1/h2 como base si existe
        titulo_base = (
            response.css('h1::text').get() or
            response.css('h2::text').get()
        )
        if titulo_base:
            partes.append(titulo_base.strip())
        # Agregar dirección si existe
        if item.get('direccion'):
            partes.append(str(item['direccion']))
        # Agregar ambientes si existe
        if item.get('ambientes'):
            partes.append(str(item['ambientes']))
        # Agregar barrio si existe
        if item.get('barrio'):
            partes.append(str(item['barrio']))
        # Si no hay nada, fallback
        if not partes:
            partes.append('Propiedad en Rosario')
        item['titulo'] = ' - '.join(partes)
