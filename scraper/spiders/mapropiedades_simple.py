...
class MAPropiedadesSimpleSpider(scrapy.Spider):
    """
    Spider optimizado de MA Propiedades - extrae desde el listado
    """
    name = 'mapropiedades_simple'
    ...
    def parse_listing(self, response):
        ...
        propiedades = response.css('div.propiedad, div[class*="property"], article, div.item')
        ...
        for prop in propiedades:
            ...
            # Ubicación
            ubicacion = (
                prop.css('[class*="ubicacion"]::text, [class*="location"]::text, [class*="address"]::text').get() or
                prop.css('.direccion::text, .barrio::text').get()
            )
            ...
            # PRECIO
            precio_text = prop.css('[class*="precio"]::text, [class*="price"]::text').get()
            if precio_text:
        ...
        # Paginación
        next_page = response.css('a.siguiente::attr(href), a[rel="next"]::attr(href), a.pagination__next::attr(href)').get()
        if next_page:
