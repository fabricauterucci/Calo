...
class RemaxSimpleSpider(scrapy.Spider):
    """
    Spider optimizado de Remax - extrae desde el listado
    """
    name = 'remax_simple'
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(
            ...
            meta={
                'selenium': True,
                'wait_for': 'div.card-property, div[class*="property"]',
                'wait_time': 5,
                'scroll': True
            }
        )
    ...
    def parse_listing(self, response):
        ...
        propiedades = response.css('div.card-property, div[class*="property-card"], article')
        ...
        for prop in propiedades:
            ...
            # Ubicación
            ubicacion = (
                prop.css('[class*="location"]::text, [class*="address"]::text').get() or
                prop.css('.card-property__location::text').get()
            )
            ...
            # PRECIO
            precio_text = prop.css('[class*="price"]::text, .card-property__price::text').get()
            if precio_text:
